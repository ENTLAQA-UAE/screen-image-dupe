...
No manager role in v1. 2. Assessment Model
Core entities:
Assessment (P0)
...
Question Bank: ability to mark questions as reusable and browse/filter them when building new Assessments. 3. AI Assessment Builder
Builder Modes (P0)
...
Basic duplicate prevention rules (P1): optional enforcement of one submission per email/employeeCode per Assessment Group. 5. Assessment-Taking Experience
Participant UI (P0)
...
Steps:
Landing: intro text, instructions, time estimate, start button.
Question pages:
...
If showResultsToEmployee = false: no scores, no narrative, only confirmation. 6. Results, Reporting & Employee Profiles
...
Future extension (P1): AI “Talent Snapshot” summarizing all assessments into strengths, risks, and development recommendations. 7. Localization & Bilingual Support
...
All static templates for emails, screens, and PDFs have EN and AR variants. 8. Security, Access & Compliance
...
Super Admin logs into a dedicated console to create Organizations and configure subscriptions.
Organization Admin receives invite email, sets password, and logs into the Org workspace. They configure branding, default language, and invite HR Admins.
HR Admin logs in and lands on a dashboard showing Assessments, Assessment Groups, and key metrics.
...
Link Expiry:
Links must handle pre-start and post-end gracefully with clear messages; no ability to answer outside the window.
Duplicate Submissions:
Participant unique links allow a single completed response; re-opened links show either the thank-you or view-only results (configurable later).
Draft vs Published Assessments:
Only Assessments marked as “Ready/Published” can be used to create Assessment Groups.
UI/UX Highlights
...
Samples of assessments for guidance only:
Below are detailed description of assessments

For other questions:

Target audience: will be for all employees, can be used for different purposes e.g promotion, selection,
Reason: Promotion, development, selection
Expecting results: to help us in recognizing the best fit and on the other hand developing personal development plan for development
Yes better to be scoring specially for selection and promotion
...
Overall solid. We’ll now translate the PRD into something AI agents and engineers can execute on: clear entities, APIs, and workflows.
Below is a concise “Specs for AI Agents” layer on top of Jadarat Assess. It assumes modern REST/JSON and LLM capabilities.
...
2. High-Level API Surface (for AI Agents)
   Describe endpoints and their intent; exact URLs/methods can be adjusted, but behavior must hold.
...
Output: structured narrative JSON, e.g.:
summary, strengths[], risks[], developmentRecommendations[].
...
8. Execution Order for an AI “Builder Agent”
   ...
   Call POST /assessments with base metadata and type = COGNITIVE.
   Based on user input, fill config with subdomains, questionCount, difficulty, etc.
   Call POST /ai/builder/generate-questions to get question set.
   Persist questions via POST /assessments/{assessmentId}/questions (or bulk endpoint).
   Optionally save selected questions to bank.
   Mark Assessment as READY via /publish.
   Optionally create an Assessment Group and invitations.
