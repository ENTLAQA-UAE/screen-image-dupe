...
9. [Milestones & Sequencing](#milestones--sequencing)
10. [Assessment Types Reference](#assessment-types-reference)
11. [AI Agent Specifications](#ai-agent-specifications)
...
| Story | Description |
|-------|-------------|
| SA-1 | As a Super Admin, I want to create and manage multiple client organizations, so that each enterprise has an isolated and secure workspace. |
| SA-2 | As a Super Admin, I want to configure subscriptions, usage limits, and manual billing cycles per organization, so that we can manage commercial terms and invoicing. |
| SA-3 | As a Super Admin, I want to define global product settings (available assessment types, language defaults, feature flags), so that all organizations operate within controlled boundaries. |

### Organization Admin

| Story | Description |
|-------|-------------|
| OA-1 | As an Organization Admin, I want to configure my organization profile and branding (name, logo, primary color, primary language), so that employees see a consistent branded experience. |
| OA-2 | As an Organization Admin, I want to invite and manage users (HR Admins) and assign roles, so that only authorized staff can design and run assessments. |
| OA-3 | As an Organization Admin, I want to monitor subscription usage (assessments created, Assessment Groups, participant volume), so that I can manage capacity and coordinate with Jadarat on renewals/upgrades. |

### HR Admin - Assessment Builder

| Story | Description |
|-------|-------------|
| HR-1 | As an HR Admin, I want to choose between creating a Graded Quiz or an Assessment/Profile, so that I can support both test-style and profile-style use cases. |
| HR-2 | As an HR Admin, I want to select Quick setup (generic) or Advanced setup (type-specific), so that I can either move fast or fine-tune psychometric parameters. |
| HR-3 | As an HR Admin, I want to specify the assessment language (EN or AR) and have AI generate questions and reports accordingly, so that content is fully localized. |
| HR-4 | As an HR Admin, I want AI to generate questions, answer keys, scoring logic, and result schemas from my description and selected type, so that I don't need deep psychometrics expertise. |
| HR-5 | As an HR Admin, I want to manually add, edit, reorder, and delete questions, and store them in a reusable question bank, so that I can refine and reuse content. |

### HR Admin - Assessment Types

| Story | Description |
|-------|-------------|
| HR-6 | As an HR Admin, I want specific configuration fields for Cognitive assessments (subdomains, difficulty, question count), so that I can target relevant cognitive abilities. |
| HR-7 | As an HR Admin, I want specific fields for Personality assessments (model, traits, question count), so that I can create scientifically grounded personality profiles. |
| HR-8 | As an HR Admin, I want specific fields for Behavioral (FBA-style) assessments (domains, behavior dimensions) and SJT (competencies, number of scenarios), so that I can reflect workplace behavior and scenarios accurately. |
| HR-9 | As an HR Admin, I want specialized options for language assessments (skills: grammar, vocabulary, reading; question distribution; CEFR-like level), so that I can align with language requirements. |
| HR-10 | As an HR Admin, I want to configure for each assessment whether it is graded, whether to show results to employees, and whether to allow PDF downloads, so that I align with internal HR policy. |

### HR Admin - Assessment Groups & Invitations

| Story | Description |
|-------|-------------|
| HR-11 | As an HR Admin, I want to create Assessment Groups that reference a base Assessment and define a title and description (e.g., "DESC - Q1 - IT Team"), so that I can run assessment campaigns per cohort. |
| HR-12 | As an HR Admin, I want to set start and end dates for an Assessment Group, so that employee links are valid only within that window. |
| HR-13 | As an HR Admin, I want to add participants to an Assessment Group with name, department, job title, employee code, and email, so that I can track identified participation. |
| HR-14 | As an HR Admin, I want the system to generate unique UUID-based links per participant, so that each link is secure, traceable, and bound to a single person and group. |
| HR-15 | As an HR Admin, I want to enable a group link for an Assessment Group that allows any employee to self-identify and complete the assessment, so that I can support open or very large cohorts. |

### HR Admin - Results & Reporting

| Story | Description |
|-------|-------------|
| HR-16 | As an HR Admin, I want to view overall statistics per Assessment Group (invited, started, completed, average score, distribution), so that I can measure engagement and performance. |
| HR-17 | As an HR Admin, I want to open an individual participant's detailed report, including raw scores, subscale scores, and AI-generated narrative interpretation, so that I can make informed talent decisions. |
| HR-18 | As an HR Admin, I want to control whether employees see their results for each Assessment, so that I can hide sensitive outcomes where needed. |
| HR-19 | As an HR Admin, I want to export PDF reports and tabular data (CSV) per group, so that I can share with stakeholders and use in HR processes. |

### HR Admin - Employees

| Story | Description |
|-------|-------------|
| HR-20 | As an HR Admin, I want an Employees tab listing all participants with their key data, so that I can easily find individuals. |
| HR-21 | As an HR Admin, I want to open an employee profile to see chronological history of all assessments they completed, with date, type, and status/score, so that I can understand their trajectory. |
| HR-22 | As an HR Admin, I want to open any historic report from an employee profile, so that I can review context without navigating through older Assessment Groups. |

### Employee (Participant)

| Story | Description |
|-------|-------------|
| E-1 | As an Employee, I want to open my unique assessment link (or group link) and see a clean, branded, bilingual interface, so that I trust and understand the process. |
| E-2 | As an Employee, I want to see clear instructions, time limits (if any), and progress indicators, so that I can manage my time and complete the assessment successfully. |
| E-3 | As an Employee, I want to see my results and download a PDF report when allowed, so that I know my strengths and areas for development. |
| E-4 | As an Employee, when results are not shared, I want a clear thank-you screen confirming submission, so that I know my responses are recorded. |
...
**HR Admin:**
- Full operational access within their Organization for assessments, groups, employees, and reports.
...
| type | Enum | COGNITIVE, PERSONALITY, BEHAVIORAL, SJT, ENGLISH, GENERIC_QUIZ, GENERIC_PROFILE |
| graded | Boolean | Whether the assessment is graded |
| language | Enum | EN/AR |
| defaultQuestionCount | Integer | Default number of questions |
| showResultsToEmployee | Boolean | Whether to show results to employees |
| allowEmployeePdfDownload | Boolean | Whether employees can download PDF |
| allowPreview | Boolean | Whether preview is allowed |
| AIFeedbackEnabled | Boolean | Whether AI feedback is enabled |
...
| startDate | DateTime | Start date for the assessment window |
| endDate | DateTime | End date for the assessment window |
| status | Enum | DRAFT, ACTIVE, CLOSED |
| groupLinkEnabled | Boolean | Whether group link is enabled |
| groupLinkUrl | String | URL for group link |
| createdBy | UUID | User who created it |
| createdAt | DateTime | Creation timestamp |
| updatedAt | DateTime | Last update timestamp |
...
|-------|------|-------------|
| id | UUID | Primary identifier |
| assessmentGroupId | UUID | Foreign key to Assessment Group |
| employeeId | UUID (nullable) | Foreign key to Employee (nullable for group link first-time) |
| name | String | Participant name |
| department | String | Department |
| jobTitle | String | Job title |
| employeeCode | String | Employee code |
| email | String | Email address |
| participantUuid | UUID | Unique UUID used in invite link |
| status | Enum | INVITED, STARTED, COMPLETED, EXPIRED |
| languageResolved | Enum | EN/AR based on Assessment and possibly user choice |
...
|-------|------|-------------|
| id | UUID | Primary identifier |
| assessmentId or bankId | UUID | Foreign key |
| type | Enum | MCQ_SINGLE, MCQ_MULTI, LIKERT, OPEN_TEXT, SJT_MATRIX, etc. |
| text | String | Question text |
| options | JSON | Answer options |
| correctAnswer | JSON | Correct answer(s) if graded |
| scoringLogic | JSON | Scoring logic |
| metadata | JSON | Tags, difficulty, subdomain |
...
##### Personality/Psychometric
- Model selection (e.g., Big Five-like, DISC-like) for internal logic
...
**If showResultsToEmployee = false:**
- No scores, no narrative, only confirmation
...
1. HR Admin clicks "Create Assessment"
2. Chooses category: Graded Quiz or Assessment/Profile
3. Chooses type: Cognitive, Personality, Behavioral, SJT, English, Generic Quiz, Generic Profile
4. Default mode: Advanced setup (Type-specific). Option to switch to Quick setup
5. Fills in necessary fields (description, language, question count, subdomains/traits/etc., showResultsToEmployee, allowEmployeePdfDownload, AIFeedbackEnabled)
6. Triggers AI generation; system displays a questions list grouped by sections
7. HR Admin edits, reorders, and saves. Assessment status becomes "Ready"

#### Step 2: Create Assessment Group

1. From an Assessment, HR Admin clicks "Create Group"
2. Sets name (e.g., "DESC - Q1 - IT Team"), description, startDate, endDate, groupLinkEnabled
3. Adds participants via form and optionally CSV import
4. System generates UUID links for each participant and (if enabled) a group link
5. Group status becomes "Active" at startDate
...
1. HR Admin opens the Group results dashboard to see completion and performance summary
2. Clicks on any participant to open a detailed report, including AI interpretation
3. If relevant, export PDFs or CSV for further analysis

#### Step 5: Employee Profiles

1. HR Admin navigates to the Employees tab
2. Searches for an employee and opens their profile
3. Views list of all assessments and can open any historic report from there
...
This section provides technical specifications for AI agents and engineers to execute on the requirements.
...
| id | UUID |
| organizationId | UUID |
| role | Enum (SUPER_ADMIN, ORG_ADMIN, HR_ADMIN) |
| name | String |
| email | String |
| languagePreference | Enum (EN/AR) |
| status | Enum |
...
| name | String |
| description | String |
| category | Enum (GRADED_QUIZ, PROFILE) |
| type | Enum (COGNITIVE, PERSONALITY, BEHAVIORAL, SJT, ENGLISH, GENERIC_QUIZ, GENERIC_PROFILE) |
| language | Enum (EN, AR) |
| graded | Boolean |
| showResultsToEmployee | Boolean |
| allowEmployeePdfDownload | Boolean |
...
|-------|------|
| id | UUID |
| assessmentId or bankId | UUID |
| language | Enum (EN/AR) |
| itemType | Enum (MCQ_SINGLE, MCQ_MULTI, LIKERT, OPEN_TEXT, SJT_CHOICE, SJT_RANKING, etc.) |
| text | String |
| options | JSON[] |
| correctAnswer / correctAnswers | JSON (if graded) |
| scoringLogic | JSON (e.g., option -> score, trait weighting) |
| metadata | JSON (difficulty, subdomain/trait, tags) |
...
|-------|------|
| id | UUID |
| assessmentGroupId | UUID |
| employeeId | UUID (nullable until matched) |
| name | String |
| department | String |
| jobTitle | String |
| employeeCode | String |
...
| participantId | UUID |
| assessmentId | UUID |
| languageResolved | Enum (EN/AR) |
| startedAt | DateTime |
| submittedAt | DateTime |
| answers | JSON (array of {questionId, value}) |
| scores | JSON (overall + subscales) |
| aiReport | JSON (narrative sections) |
| resultVisibleToEmployee | Boolean (snapshot) |
...
|----------|--------|-------------|
| `/organizations` | POST | Create new organization with profile + subscription meta |
| `/organizations/{orgId}/users` | POST | Create Organization Admin or HR Admin |
| `/organizations/{orgId}/usage` | GET | Returns usage counters for dashboards and guardrails |

#### 2.2 Assessment Management

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/organizations/{orgId}/assessments` | POST | Create a new Assessment in DRAFT |
| `/organizations/{orgId}/assessments/{assessmentId}` | GET | Retrieve Assessment definition and questions |
| `/organizations/{orgId}/assessments/{assessmentId}` | PATCH | Update metadata, config, toggles |
| `/organizations/{orgId}/assessments/{assessmentId}/publish` | POST | Set status to READY (validation: has questions, config valid) |
...
| Endpoint | Method | Description |
|----------|--------|-------------|
| `/ai/builder/generate-questions` | POST | Generate questions based on type, category, language, description, questionCount, config |
| `/ai/reports/generate` | POST | Generate structured narrative from assessmentType, language, scores, optional context |

**Input for `/ai/builder/generate-questions`:**
```json
{
  ...
  "questionCount": 30,
  "config": {
    "subdomains": ["numerical", "verbal", "logical"],
    "difficulty": "mixed"
  }
}
```

**Output for `/ai/reports/generate`:**
```json
{
  "summary": "...",
  "strengths": ["..."],
...
| Endpoint | Method | Description |
|----------|--------|-------------|
| `/organizations/{orgId}/questions/bank` | POST | Save an existing Question into the bank with tags |
| `/organizations/{orgId}/questions/bank` | GET | Query bank by tags (type, domain, difficulty, language) |
...
**Logic:**
1. Look up existing Employee by employeeCode or email
2. If exists, reuse employeeId; else, create Employee
3. Create Participant with generated participantUuid, status = INVITED
...
```
1. Validate time window, status, and map to Assessment & Group
2. Resolve languageResolved from Assessment
3. If allowed, return Assessment structure (questions + UI config) and move Participant status to STARTED
...
2. Score using Question.scoringLogic and Assessment config
3. Persist Attempt with scores and answers
4. If assessment.aiFeedbackEnabled, call /ai/reports/generate, persist aiReport
5. Update Participant.status = COMPLETED (if within window) else EXPIRED logic if out of time
...
| Endpoint | Method | Description |
|----------|--------|-------------|
| `/organizations/{orgId}/groups/{groupId}/summary` | GET | Returns: counts (invited/started/completed/expired), average scores, distribution |
| `/organizations/{orgId}/groups/{groupId}/participants` | GET | Returns: tabular view of participants with status and key metrics |

#### 5.2 Individual Report

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/organizations/{orgId}/participants/{participantId}/report` | GET | Returns Participant details, Assessment/Group metadata, scores and aiReport |
| `/organizations/{orgId}/participants/{participantId}/report.pdf` | GET | Generates or serves cached bilingual PDF based on the JSON report |

#### 5.3 Employee Profile

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/organizations/{orgId}/employees` | GET | List/search employees |
| `/organizations/{orgId}/employees/{employeeId}` | GET | Core profile + list of all attempts: each with assessment name, group, date, overall score |
| `/organizations/{orgId}/employees/{employeeId}/attempts` | GET | For deeper programmatic access to all their assessment data |

### 6. Guardrails & Business Rules for Agents

AI/LLM agents that generate questions and reports must respect:

1. **Language:** Use assessment.language exclusively (no mixing EN/AR in same question unless explicitly requested)
...
3. **Question count & structure:** Stay within questionCount and per-domain allocations from config

4. **Safety & Bias:** Avoid discriminatory or culturally inappropriate content; keep scenarios generic or role-relevant and neutral
...
|---------|------|--------|---------|
| 1.0 | 2024 | Jadarat Team | Initial document creation |
